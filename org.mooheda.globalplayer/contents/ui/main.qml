...
Item {
        ...
        onNewData: function(sourceName, data) {
            ...
            if (sourceName.indexOf("GetNowPlaying") !== -1) {
                try {
                    ...
                    var currentTrack = nowArtist + " — " + nowTitle
                    if (currentTrack.trim() !== " — " && currentTrack !== lastNotifiedTrack && playState === "Playing") {
    ...
    function sendNotification(trackInfo) {
        var cmd = "notify-send \"Global Player\" \"" + trackInfo.replace(/\"/g, "\\\"") + "\" --icon=audio-headphones"
        execDS.connectSource(cmd)
    }
    ...
    // Compact panel UI - Cover art + controls only
    Plasmoid.compactRepresentation: Item {
        ...
        Row {
            ...
            Row {
                ...
                PC3.ToolButton {
                    text: playState === "Playing" ? "⏸" : "▶"
                    onClicked: playState === "Playing" ? qdbusCall("Pause", []) : playCurrent()
                    ToolTip.text: playState === "Playing" ? "Pause" : "Play"
                }
                ...
                PC3.ToolButton {
                    text: "⏹"
                    onClicked: { qdbusCall("Pause", []); playState = "Stopped" }
                    ToolTip.text: "Stop"
                }
            }
        }

        // Wheel scrolling for station switching
        WheelHandler {
            target: this
            onWheel: {
        ...
        MouseArea {
            ...
            onClicked: if (mouse.button === Qt.RightButton) contextMenu.open()
        }
